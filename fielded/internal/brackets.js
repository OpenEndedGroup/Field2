...
function makePathForHandles(h1, h2)
{
	...
	if (ps)
	{
		...
		h1.on("delete", function(x) { path.remove() })
		h2.on("delete", function(x) { path.remove() })
...
raph.clear()
//mp = makePathForHandles(cm.getLineHandle(1), cm.getLineHandle(3))
//mp = makePathForHandles(cm.getLineHandle(3), cm.getLineHandle(4))
//mp = makePathForHandles(cm.getLineHandle(4), cm.getLineHandle(15))
...
cm.on("change", function(x,c) { updateAllBrackets() })
cm.on("fold", function(x,c) { updateAllBrackets() })
cm.on("unfold", function(x,c) { updateAllBrackets() })

$(window).resize(function() {updateAllBrackets()})
