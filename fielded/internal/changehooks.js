...
cm.on("change", function (cm, change)
{
	...
	if (cm.currentbox && cm.currentproperty)
    	_messageBus.publish("toField.text.updated", {box:cm.currentbox, property:cm.currentproperty, text:cm.getValue()})
})
...
_messageBus.subscribe("selection.changed", function (d,e)
{
    ...
    if (cm.currentbox)
    {
        ...
        cookie.currentpos = cm.getCursor()
   	    _messageBus.publish("toField.store.cookie", {box:cm.currentbox, property:cm.currentproperty, "cookie":cookie})
