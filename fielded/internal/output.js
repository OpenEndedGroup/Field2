...
function appendRemoteOutputToLine(line, text, checkClass, lineClass, append)
{
    ...
    if (found!=null)
    {
        ...
        $(d).animate({scrollTop:$(d)[0].scrollHeight}, 0)
        $(d).css({opacity: 1})
        $(d).stop().animate({opacity: 0.75},500)
        found.changed()
    }
    else
    {
    	...
    	console.log(d)
	    bm = cm.addLineWidget(lh, d, {showIfHidden:true,handleMouseEvents:false})
    }
...
//appendRemoteOutputToLine(3, s, "Field-remoteOutput", "Field-remoteOutput-error", true)
//appendRemoteOutputToLine(3, s, "Field-remoteOutput-error", "Field-remoteOutput", true)
...
globalCommands.push({"name":"clear outputs", "info":"clear all output areas in this editor", "callback":function (){
