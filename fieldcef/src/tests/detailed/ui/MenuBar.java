...
public class MenuBar extends JMenuBar {
	...
	public MenuBar(Frame owner, CefBrowser browser, ControlPanel control_pane, DownloadDialog downloadDialog, CefCookieManager cookieManager) {
		owner_ = owner;
		browser_ = browser;
					...
					public void onFileDialogDismissed(CefBrowser browser, Vector<String> filePaths) {
						if (!filePaths.isEmpty()) {
							try {
								...
								browser_.executeJavaScript("alert(\"Can't save file\");", control_pane_
									    .getAddress(), 0);
							}
						}
					}
				};
				browser_.runFileDialog(FileDialogMode.FILE_DIALOG_SAVE, owner_.getTitle(), "index.html", null, callback);
			}
		});
		...
		showCookies.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				CookieManagerDialog cookieManager = new CookieManagerDialog(owner_, "Cookie Manager", cookieManager_);
		...
		showPlugins.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				WebPluginManagerDialog pluginManager = new WebPluginManagerDialog(owner_, "Plugin Manager");
