...
public abstract class CefWebPluginManager {
	...
	/**
	 * Cause the plugin list to refresh the next time it is accessed regardless of whether it has already been loaded. Can be called on any thread
	 * in the browser process.
	 */
	public abstract void refreshPlugins();

	/**
	 * Add a plugin path (directory + file). This change may not take affect until after refreshPlugins() is called. Can be called on any thread
	 * in the browser process.
	 */
	public abstract void addPlugin(String path);

	/**
	 * Add a plugin directory. This change may not take affect until after refreshPlugins() is called. Can be called on any thread in the browser
	 * process.
	 */
	public abstract void addPluginDirectory(String directory);

	/**
	 * Remove a plugin path (directory + file). This change may not take affect until after CefRefreshWebPlugins() is called. Can be called on any
	...
	/**
	 * Unregister an internal plugin. This may be undone the next time CefRefreshWebPlugins() is called. Can be called on any thread in the
	 * browser process.
	 */
