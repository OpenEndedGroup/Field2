...
public class OS {
	...
	private static final OSType getOSType() {
		if (osType == OSType.OSUndefined) {
			...
			if (os.startsWith("windows")) osType = OSType.OSWindows;
			else if (os.startsWith("linux")) osType = OSType.OSLinux;
			else if (os.startsWith("mac")) osType = OSType.OSMacintosh;
			else osType = OSType.OSUnknown;
		}
		return osType;
	}
}
