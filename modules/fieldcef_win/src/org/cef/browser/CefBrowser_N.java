...
/**
 * This class represents all methods which are connected to the native counterpart CEF. The visibility of this class is "package". To create a new
 * CefBrowser instance, please use CefBrowserFactory.
 */
abstract public class CefBrowser_N extends CefNativeAdapter implements CefBrowser {
	 ...
	 */
	protected final void createDevTools(CefBrowser parent, CefClientHandler clientHandler, long windowHandle, boolean transparent, Canvas canvas) {
		if (getNativeRef("CefBrowser") == 0 && !isPending_) {
			try {
				isPending_ = N_CreateDevTools(parent, clientHandler, windowHandle, transparent, canvas);
			} catch (UnsatisfiedLinkError err) {
				err.printStackTrace();
			}
		}
...
//			N_Close();
	...
	private final native boolean N_CreateBrowser(CefClientHandler clientHandler, long windowHandle, String url, boolean transparent, Canvas canvas, CefRequestContext context);

	private final native boolean N_CreateDevTools(CefBrowser parent, CefClientHandler clientHandler, long windowHandle, boolean transparent, Canvas canvas);
