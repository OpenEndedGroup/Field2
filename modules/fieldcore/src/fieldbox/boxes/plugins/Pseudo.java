...
public class Pseudo extends Box {
	...
	static public Dict.Prop<FunctionOfBoxValued<Herer>> here = new Dict.Prop<FunctionOfBoxValued<Herer>>("here").doc(" `_.here.x` returns true if box `_` has a property `x` ")
		.toCannon()
		.type();
	...
	static public Dict.Prop<FunctionOfBoxValued<WithID>> withID = new Dict.Prop<>("withID").toCannon()
		.type()
		.doc("`_.withID['abc']` returns any box with ID `abc` _below_ this box. If you want to search everywhere, try `_.root.withID.x`. ID's are unique (across the universe).");
