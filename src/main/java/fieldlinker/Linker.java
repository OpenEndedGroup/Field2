...
public class Linker extends GuardingDynamicLinkerExporter implements GuardingDynamicLinker/*, GuardingTypeConverterFactory*/ {
    ...
    public GuardedInvocation getGuardedInvocation(LinkRequest linkRequest, LinkerServices linkerServices) throws Exception {
        ...
        if (linkRequest.getCallSiteDescriptor().getOperation()
        ...
        } else if (linkRequest.getCallSiteDescriptor().toString().startsWith("SET:ELEMENT|PROPERTY(Object,Object,double)")) {
            Object rec = linkRequest.getReceiver();
            if (rec instanceof AsMap_slots) {
                if (debug)
                    System.err.println(" linking AsMap_slots.java/setSlot " + rec);
                MethodHandle get = MethodHandles.lookup()
                        .findVirtual(implementingClassForSlots(rec.getClass()), "setSlot", MethodType.methodType(Object.class, Object.class, Double.TYPE));

                return new GuardedInvocation(get, Guards.isInstance(rec.getClass(), MethodType.methodType(Boolean.TYPE, Object.class)));
            }
        } else if (linkRequest.getCallSiteDescriptor().toString().startsWith("SET:ELEMENT|PROPERTY(Object,double,double)")) {
            Object rec = linkRequest.getReceiver();
            if (rec instanceof AsMap_slots) {
                if (debug)
                    System.err.println(" linking AsMap_slots.java/setSlot " + rec);
        ...
        } else if (linkRequest.getCallSiteDescriptor().toString().startsWith("SET:ELEMENT|PROPERTY(Object,double,Object)")) {
            Object rec = linkRequest.getReceiver();
            if (rec instanceof AsMap_slots) {
                if (debug)
                    System.err.println(" linking AsMap_slots.java/setSlot " + rec);
        ...
        } else if (linkRequest.getCallSiteDescriptor().toString().startsWith("GET:ELEMENT|PROPERTY|METHOD")) {
            Object rec = linkRequest.getReceiver();
            if (debug)
                System.err.println("link request has " + linkRequest.getArguments().length + " argument");
            ...
            if (linkRequest.getArguments().length == 2) {
                ...
                if (linkRequest.getArguments()[1].getClass().isPrimitive()) {
                    ...
                    if (rec instanceof AsMap_slots) {
                        if (debug)
                            System.err.println(" linking AsMap_slots.java/FAKE property get " + rec);
                        MethodHandle get = MethodHandles.lookup()
                                .findVirtual(rec.getClass(), "getSlot", MethodType.methodType(Object.class, Object.class));
                        return new GuardedInvocation(get, Guards.isInstance(rec.getClass(), MethodType.methodType(Boolean.TYPE, Object.class)));
                    }
                    if (rec instanceof AsMap) {
                        if (debug)
