        ...
        implements IO.Loaded // the drawer is initialized after all the properties are loaded
{
    ...
    protected Map<String, Function<Box, FLine>> defaultdrawsLines(Dict.Prop<Map<String, Function<Box, FLine>>> k) {
        ...
        r.put("__outline__", new Cached<Box, Object, FLine>((box, previously) -> {
            ...
            FLine f = arc(((PadsBox) box).head().properties.get(frame), ((PadsBox) box).tail().properties.get(frame));

            f = thickenArc(f, ((PadsBox) box).head().properties.get(frame),
                           ((PadsBox) box).tail().properties.get(frame));
            ...
            f.attributes.put(thicken,
                             new BasicStroke(selected ? 3 : 0.5f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER));

            f.attributes.put(filled, true);
            f.attributes.put(stroked, true);

            return f;
        }, (box) -> new Triple<Boolean, Rect, Rect>(properties.get(Mouse.isSelected),
                                                    ((PadsBox) box).head().properties.get(frame),
                                                    (((PadsBox) box).tail().properties.get(frame)))));
        ...
        r.put("__middle_nub__", new Cached<Box, Object, FLine>((box, previously) -> {
            ...
            f.attributes.put(thicken,
                             new BasicStroke(selected ? 16 : 0.5f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER));
    ...
    public Vec2 middleOf(FLine f) {
        ...
        return FLinesAndJavaShapes.evaluateCubicFrame(m.to.toVec2(), c.c1.toVec2(), c.c2.toVec2(), c.to.toVec2(), 0.5f,
                                                      new Vec2());
    }
    ...
    protected Box head() {
        return properties.get(TopologyBox.head).get(this);
    }

    protected Box tail() {
        return properties.get(TopologyBox.tail).get(this);
    }
