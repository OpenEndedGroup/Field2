...
class SlitScanner(val source: String) {
    ...
    init {
        var files = Files.list(File(source).toPath()).filter { it.toString().endsWith(".jpg") && !it.fileName.startsWith(".") }.sorted().collect(
            Collectors.toList())
            ...
            pool.submit {
                ...
                try {
                    for (x in 0 until w) {
                        ...
                        o.put(3 * y * w + 3 * x + 0, (Math.max(0f, Math.min(1f, oo[0])) * 255).toInt().toByte())
                        o.put(3 * y * w + 3 * x + 1, (Math.max(0f, Math.min(1f, oo[1])) * 255).toInt().toByte())
                        o.put(3 * y * w + 3 * x + 2, (Math.max(0f, Math.min(1f, oo[2])) * 255).toInt().toByte())
