...
import trace.util.PointCloudFlann
import java.io.File
...
//                println("normal! $it.normal")
    ...
    fun toKNNFLine(fl: FLine, k : Int) {
        val f = PointCloudFlann(this)
        points.forEachIndexed { i, t ->
            for(q in f.closestN(i, k))
            {
                if (q==i) continue
                fl.moveTo(t.first)

                fl.node().attributes.put(StandardFLineDrawing.color, t.second)
                if (t.third != null)
                    fl.node().attributes.put(normal, t.third)

                val t2 = points[q]
                fl.lineTo(t2.first)

                fl.node().attributes.put(StandardFLineDrawing.color, t2.second)
                if (t2.third != null)
                    fl.node().attributes.put(normal, t2.third)
