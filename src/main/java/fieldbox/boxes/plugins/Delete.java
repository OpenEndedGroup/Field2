...
import fieldcef.plugins.PadsBox;
...
public class Delete extends Box {
		...
		properties.put(MarkingMenus.menuSpecs, (event) -> {
			if (selected().findAny()
				...
				really.items.put(MarkingMenus.Position.N2, new MarkingMenus.MenuItem(
					"Really, delete " + count + " " + name(selected()) + (count == 1 ? "" : "es") + "?", () -> {
					{
						Stream<Box> all = selected();
						all.forEach(bb -> Callbacks.delete(bb));
					}
				...
				spec.items.put(MarkingMenus.Position.S2,
					new MarkingMenus.MenuItem("Delete " + count + " " + name(selected()) + (count == 1 ? "" : "es"), () -> {
					}).setSubmenu(really));
	...
	private String name(Stream<Box> selected) {
		if (selected.filter(x -> !(x instanceof DispatchBox) && !(x instanceof PadsBox) && !(x instanceof TopologyBox)).count() > 0)
			return "box";
		else
			return "connection";
	}
