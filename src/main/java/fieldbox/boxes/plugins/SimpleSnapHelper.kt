...
class SimpleSnapHelper(private val inside: Box, val THRESHOLD: Int = 10) {
    ...
    fun help(ongoing: Collection<Triple<Box, Rect, Rect>>): List<FLine> {
        ...
        val min = ongoing.minByOrNull { it: Triple<Box, Rect, Rect> -> it.second.x }!!.third.x
        val max = ongoing.maxByOrNull { it: Triple<Box, Rect, Rect> -> it.second.x + it.second.w }!!.third.xw
