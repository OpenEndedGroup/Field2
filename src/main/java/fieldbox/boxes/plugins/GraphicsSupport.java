...
public class GraphicsSupport extends Box {
    ...
    static public Dict.Prop<IdempotencyMap<Runnable>> _shadersReloadedCallback = new Dict.Prop<>("_shadersReloadedCallback").set(IO.dontCopy, true).autoConstructs(() -> new IdempotencyMap<Runnable>(Runnable.class));
    ...
    public GraphicsSupport(Box root_unused) {
        ...
        properties.put(Commands.commands, () -> {
            ...
            if (s != null) m.put(new Pair<>("Reload shader", "Reloads all " + s.size() + " shader" + (s
                ...
                reload(box, s);
                box.properties.getOrConstruct(_shadersReloadedCallback).values().forEach(x -> x.run());
            });
    ...
    static public void reload(Box b, List<Shader> s) {
        ...
        for (Shader ss : s) {
            ...
            {
                String v = b.properties.get(ComputeShaderSupport.compute);

                Shader.Source source = sources.get(Shader.Type.compute);
                if (source == null && v != null && v.trim().length() > 0)
                    ss.addSource(Shader.Type.compute, () -> v);
