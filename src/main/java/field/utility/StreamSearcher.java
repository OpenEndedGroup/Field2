...
public class StreamSearcher {
	...
	/**
	 * Searches for the next occurrence of the pattern in the stream, starting from the current stream position. Note that the position of the stream is changed. If a match is found, the stream
	 * points to the end of the match -- i.e. the byte AFTER the pattern. Else, the stream is entirely consumed. The latter is because InputStream semantics make it difficult to have another
	 * reasonable default, i.e. leave the stream unchanged.
	 *
	 * @return bytes consumed if found, -1 otherwise.
	 */
	public long search(ByteBuffer stream) {
		long bytesRead = 0;
	...
	/**
	 * Builds up a table of longest "borders" for each prefix of the pattern to find. This table is stored internally and aids in implementation of the Knuth-Moore-Pratt string search.
	 * <p>
