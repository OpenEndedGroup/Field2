 ...
 */
public class SaverFBOFramePacked {
    ...
    public SaverFBOFramePacked(int numWorkers, String prefix, FBO fbo) {
    ...
    private void getImage(Pair<ByteBuffer, ByteBuffer> storage) {
        ...
        glReadPixels(0, 0, 3840, 2205, GL12.GL_BGR, GL_UNSIGNED_BYTE, storage.first);
    ...
    private Callable<Pair<ByteBuffer, ByteBuffer>> makeWorker(final Pair<ByteBuffer, ByteBuffer> storage, final String filename) {
        return new Callable<Pair<ByteBuffer, ByteBuffer>>() {
            public Pair<ByteBuffer, ByteBuffer> call() throws Exception {
                ...
                {
                    storage.first.position(y*(3840)*3);
                    storage.first.limit(y*3840*3+1920*3);

                    storage.second.position((1080+45)*1920*3+(1080-y-1)*1920*3);
                    storage.second.limit((1080+45)*1920*3+(1080-y-1+1)*1920*3);

                    storage.second.put(storage.first);
                    storage.second.clear();

                    storage.first.position(y*(3840)*3+1920*3);
                    storage.first.limit(y*3840*3+1920*3*2);

                    storage.second.position((1080-y-1)*1920*3);
                    storage.second.limit((1080-y-1+1)*1920*3);
                ...
                try {
                    new SlowJPEG().compress(filename, storage.second, 1920, 2205);
//					j2.compress(filename, storage.first, width, height);
                } catch (Throwable t) {
                    System.err.println(" -- exception thrown in compress for :" + filename + " " + storage + " " + 1920 + " " + 2205);
                    t.printStackTrace();
                }
                return storage;
    ...
    private Pair<ByteBuffer, ByteBuffer> newStorage() {
        return new Pair<>(ByteBuffer.allocateDirect(3840 * 1080* 4), ByteBuffer.allocateDirect(1920 * 2205* 4));
    }
