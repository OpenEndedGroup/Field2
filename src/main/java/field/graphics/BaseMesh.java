...
public class BaseMesh extends Scene implements Scene.Perform {
	...
	/**
	 * returns a new BaseMesh that draws up to numPoints vertices and numElement triangles with adjacency
	 * <p>
	 * call vertex() to get a FloatBuffer into which you can put vertex data, aux(...) to get FloatBuffers to put aux data and call elements() to
	 * connect vertices together with line segments
	 * <p>
	 ...
	 * will give an L shaped, two line segment shape.
	 */
	static public BaseMesh triangleAdjacencyList(int numPoints, int numElements) {
		return standard(numPoints, numElements, GL_TRIANGLES_ADJACENCY, 6);
	}
	...
	public String toString() {
		...
		if (this.elements == null)
			return "Mesh(" + this.vertex(true).limit() / 3 + "("+getVertexLimit()+") points)";

		int d = this.elements.getDimension();
		int num = this.elements.ints(true)
			.limit() / d;
		if (d == 2)
			return "Mesh(" + num + " lines / " + this.vertex(true).limit() / 3 +"("+getVertexLimit()+") vertices)";

		return "Mesh(" + num + " triangles / " + this.vertex(true).limit() / 3 + "("+getVertexLimit()+") vertices)";
