    ...
    public void goThreaded() {
        if (shareContextAtConstruction != null)
            throw new IllegalArgumentException(
                    " can't multithread a window if it's sharing a context on construciton. Set Window.shareContext to null before creating window");
...
//        if (Main.os== Main.OS.windows)
    ...
    protected void updateScene() {
        ...
        try {
            ...
            GraphicsContext.getContext().stateTracker.scissor.set(
                    new int[]{0, 0, w * getRetinaScaleFactor(), h * getRetinaScaleFactor()});
            GraphicsContext.getContext().stateTracker.fbo.set(0);
            GraphicsContext.getContext().stateTracker.shader.set(0);
            GraphicsContext.getContext().stateTracker.blendState.set(
                    new int[]{GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA});
...
//                    System.out.println("key> " + key + "/" + scancode + "/" + action);
                    ...
                    next.windowsScancodes.addAll(keyboardState.windowsScancodes);
                    if (action == GLFW_RELEASE)
                        next.windowsScancodes.removeIf((p) -> p == scancode);
                    else if (!next.windowsScancodes.contains(scancode)) next.windowsScancodes.add(scancode);
...
//					next = modifiers.cleanModifiers(next);
            ...
            public void character(long window, int character) {
                if (window == Window.this.window) {
                    ...
                    boolean shift = ((glfwGetKey(window, GLFW_KEY_LEFT_SHIFT)) | (glfwGetKey(window,
                            GLFW_KEY_RIGHT_SHIFT))) != 0;
                    boolean alt = ((glfwGetKey(window, GLFW_KEY_LEFT_ALT)) | (glfwGetKey(window,
                            GLFW_KEY_RIGHT_ALT))) != 0;
                    boolean meta = ((glfwGetKey(window, GLFW_KEY_LEFT_SUPER)) | (glfwGetKey(window,
...
//                    System.out.println("char> "+character);
    ...
    public Saver getScreenshotter() {
        ...
        int x = 1;
        while (new File(base + Saver.pad(x)).exists()) x++;

        File prefix = new File(base + Saver.pad(x));
        prefix.mkdirs();

        return screenshotter = new Saver(getCurrentWidth(), getCurrentHeight(), 4,
    ...
    static public class KeyboardState implements HasPosition {
        ...
        public final List<Integer> windowsScancodes = new ArrayList<>(); // this is oracle's secret back-channel for scancodes used by CEF on windows
        public final long time;
        ...
        public KeyboardState clean(long window) {
            ...
            var ks = new KeyboardState(k, charsDown, time);
            ks.windowsScancodes.addAll(this.windowsScancodes);
            return ks;
        }
    }
