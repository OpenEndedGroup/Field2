...
class NewNanoHTTPD(val port: Int) {
    ...
    var handlers =
        mutableListOf<(String, Method, Map<String, String>, Map<String, List<String>>, Map<String, String>) -> Response?>()
    ...
    val knownMimeExtensions = mutableMapOf<String, String>(
        "css" to "text/css",
...
//                    print("message $p0")
        ...
        override fun serve(session: IHTTPSession?): Response {
            ...
            return if (session.getUri().toString() == "/favicon.ico") Response.newFixedLengthResponse(
                NOT_FOUND,
                null,
                ""
            )
    ...
    init {
        ...
        handlers.add { uri, _, headers, _, _ ->
            ...
            var uri2 = if (uri.endsWith("/")) uri + "index.html" else uri
            dynamicRoots.entries.stream().map { kv ->
                val f = File(kv.value() + "/" + uri2)
                if (f.exists()) {
