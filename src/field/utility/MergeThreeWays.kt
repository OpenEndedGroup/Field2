/*
 * file based on Diff Match and Patch, copyright below
 */
...
class MergeThreeWays {
     ...
     */
    protected class LinesToCharsResult(var chars1: String, var chars2: String,
     ...
     */
    @JvmOverloads
    fun diff_main(text1: String, text2: String,
                  checklines: Boolean = true): LinkedList<Diff> {
     ...
     * @return Linked List of Diff objects.
     */
    private fun diff_main(text1: String?, text2: String?,
     ...
     * @return Linked List of Diff objects.
     */
    private fun diff_compute(text1: String, text2: String,
                             checklines: Boolean, deadline: Long): LinkedList<Diff> {
        ...
        if (hm != null) {
            ...
            val diffs_a = diff_main(text1_a, text2_a,
                    checklines, deadline)
            val diffs_b = diff_main(text1_b, text2_b,
                    checklines, deadline)
            // Merge the results.
            diffs = diffs_a
    ...
    private fun diff_lineMode(text1: String, text2: String,
        ...
        val b = diff_linesToChars(text1, text2)
        text1 = b.chars1
        text2 = b.chars2
        val linearray = b.lineArray
            ...
            when (thisDiff.operation) {
                ...
                MergeThreeWays.Operation.EQUAL -> {
                    ...
                    if (count_delete >= 1 && count_insert >= 1) {
                        ...
                        pointer.previous()
                        for (j in 0..count_delete + count_insert - 1) {
                            pointer.previous()
                            pointer.remove()
                        }
                        for (newDiff in diff_main(text_delete, text_insert, false,
                                deadline)) {
                            pointer.add(newDiff)
                        }
                    }
     ...
     */
    protected fun diff_bisect(text1: String, text2: String,
                              deadline: Long): LinkedList<Diff> {
     ...
     * @return LinkedList of Diff objects.
     */
    private fun diff_bisectSplit(text1: String, text2: String,
                                 x: Int, y: Int, deadline: Long): LinkedList<Diff> {
    ...
    protected fun diff_linesToChars(text1: String, text2: String): LinesToCharsResult {
        ...
        val chars1 = diff_linesToCharsMunge(text1, lineArray, lineHash)
        val chars2 = diff_linesToCharsMunge(text2, lineArray, lineHash)
     ...
     */
    private fun diff_linesToCharsMunge(text: String, lineArray: MutableList<String>,
                                       lineHash: MutableMap<String, Int>): String {
        var lineStart = 0
        var lineEnd = -1
        var line: String
     ...
     * @param lineArray List of unique strings.
     */
    protected fun diff_charsToLines(diffs: LinkedList<Diff>,
    ...
    protected fun diff_halfMatch(text1: String, text2: String): Array<String>? {
        ...
        val hm1 = diff_halfMatchI(longtext, shorttext,
                (longtext.length + 3) / 4)
    ...
    fun diff_cleanupSemantic(diffs: LinkedList<Diff>) {
        ...
        var changes = false
        val equalities = Stack<Diff>()  // Stack of qualities.
    ...
    fun diff_cleanupEfficiency(diffs: LinkedList<Diff>) {
        ...
        var changes = false
        val equalities = Stack<Diff>()  // Stack of equalities.
        var lastequality: String? = null // Always equal to equalities.lastElement().text
        val pointer = diffs.listIterator()
     ...
     */
    fun diff_xIndex(diffs: LinkedList<Diff>, loc: Int): Int {
        var chars1 = 0
        var chars2 = 0
        var last_chars1 = 0
        var last_chars2 = 0
     ...
     */
    fun diff_prettyHtml(diffs: LinkedList<Diff>): String {
     ...
     * @return Source text.
     */
    fun diff_text1(diffs: LinkedList<Diff>): String {
     ...
     */
    fun diff_text2(diffs: LinkedList<Diff>): String {
        val text = StringBuilder()
        for (aDiff in diffs) {
            if (aDiff.operation != Operation.DELETE) {
                text.append(aDiff.text)
     ...
     */
    fun diff_levenshtein(diffs: LinkedList<Diff>): Int {
        var levenshtein = 0
        var insertions = 0
        var deletions = 0
        for (aDiff in diffs) {
     ...
     * @return Delta text.
     */
    fun diff_toDelta(diffs: LinkedList<Diff>): String {
        val text = StringBuilder()
        for (aDiff in diffs) {
            when (aDiff.operation) {
        ...
        for (d in 0..pattern.length - 1) {
            ...
            for (j in finish downTo start) {
                ...
                if (d == 0) {
                    ...
                    rd[j] = rd[j + 1] shl 1 or 1 and charMatch or (last_rd[j + 1] or last_rd[j] shl 1 or 1) or last_rd[j + 1]
                }
    ...
    protected fun patch_addContext(patch: Patch, text: String?) {
        ...
        // Add the prefix.
        val prefix = text.substring(Math.max(0, patch.start2 - padding),
                patch.start2)
        if (prefix.length != 0) {
            patch.diffs.addFirst(Diff(Operation.EQUAL, prefix))
        }
        // Add the suffix.
        val suffix = text.substring(patch.start2 + patch.length1,
                Math.min(text.length, patch.start2 + patch.length1 + padding))
        if (suffix.length != 0) {
