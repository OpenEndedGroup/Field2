...
class Mocap(val fn: String) {
    ...
    init {
        ...
        val distance = data.map { it.distance(center) }.max()!!
        val lowest = data.maxBy { it.y }!!.y
        ...
        for (p in 0 until file.num3DPoints) {
            ...
            for (f in file.frames) {
                if (f.isValid(p)) {
                    val datum = (Vec3(f.x[p].toDouble(), -f.z[p].toDouble(), f.y[p].toDouble()) * 0.17 - Vec3(center.x, center.y, center.z)) * (50.0 / distance) + Vec3(50.0, 50.0, 0.0)
