				...
				if (s != null) {
					...
					for (int i = s.length - 1; i >= 0; i--) {
						if (s[i].getFileName() != null && s[i].getFileName()
							...
							ln.add(lineTransform.apply(s[i].getLineNumber()));
							me.add(extraMessage + " " + m + " on line " + lineTransform.apply(s[i].getLineNumber()) + " in box " + s[i].getFileName());
							extraMessage = "";
					...
					if (found) {
						lineErrors.accept(new Pair<Integer, String>(ln.get(0), String.join("\n ", me)));
					}
