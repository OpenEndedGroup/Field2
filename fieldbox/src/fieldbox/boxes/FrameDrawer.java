...
public class FrameDrawer extends Box implements Drawing.Drawer {
	...
	static public final Dict.Prop<Boolean> dirty = new Dict.Prop<Boolean>("dirty").type().toCannon();
	...
	static public final Dict.Prop<Number> textScale = new Dict.Prop<>("textScale");
	...
	public FrameDrawer() {
		...
		// allow things like _box.dirty=1 to cause redraw next frame
	...
	protected void dispatchLine(FLine fline, Drawing context) {
		...
		if (fline.attributes.isTrue(hasText, false)) {
			fline.nodes.stream().filter(node -> node.attributes.has(FrameDrawer.text)).forEach(node -> {
				...
				float textScale = node.attributes.getFloat(FrameDrawer.textScale, 1f)*0.2f;
				text.map(t -> t.getFontSupport(fline.attributes.getOr(font, () -> "source-sans-pro-regular.fnt"), layerName)).ifPresent(fs -> {
	...
	protected Map<String, Function<Box, FLine>> defaultdrawsLines(Dict.Prop<Map<String, Function<Box, FLine>>> k) {
		...
		r.put("__outline__", new Cached<Box, Object, FLine>((box, previously) -> {
			...
			f.attributes.put(strokeColor, selected ? new Vec4(0, 0, 0, -1.0f) : new Vec4(0, 0, 0, 0.2f));
	...
	static public Function<Box, FLine> expires(Function<Box, FLine> wrap, int updates, float power) {
		...
		return box -> {
			if (updates < 0) return wrap.apply(box);
			FLine f = wrap.apply(box);
			f.attributes.multiply(FrameDrawer.opacity, 1, (float) Math.pow(counter[0] / (float) updates, power));
			f.modify();
			Drawing.dirty(box);
			return counter[0]-- < 0 ? null : f;
		};
