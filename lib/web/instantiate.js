...
var inside = ensureTabCalled("code", "code", function(){
    if (window.cm)
        cm.refresh()
    return undefined
});

var cm = CodeMirror(inside);
cm.setValue("");
...
cm.setOption("gutters", ["CodeMirror-lint-markers", "CodeMirror-foldgutter", "CodeMirror-linenumbers"]);
    ...
    _field.sendWithReturn("request.commands", {
        ...
        function (d, e) {
            ...
            completionFunction = function (e) {
                ...
                for (var i = 0; i < completions.length; i++) {
                    if (stripTags(completions[i].name).search(fuzzyPattern) != -1) {
                        matched = stripTags(completions[i].name).replace(fuzzyPattern, replacer);
                        m.push({
            ...
            if (completions.length > 0)
                runModal("Commands... [esc exits]", completionFunction, "Field-Modal")
        }
    );
};
...
var selectPreviousNumber = function()
{
    cc = cm.getSearchCursor(/[-+]?[0-9]*\.?[0-9]+/, cm.getCursor())
    cc.findPrevious()
    if (cc.findPrevious())
    {
        cm.setSelection(cc.from(), cc.to())
    }
    else
    {
        cc = cm.getSearchCursor(/[-+]?[0-9]*\.?[0-9]+/, {line:cm.doc.lineCount()-1, ch:cm.doc.getLine(cm.doc.lineCount()-1).length-1})
        if (cc.findPrevious())
        {
            cm.setSelection(cc.from(), cc.to())
        }
...
var selectNextNumber = function()
{
    ...
    if (cc.findNext())
    {
        ...
        cc = cm.getSearchCursor(/[-+]?[0-9]*\.?[0-9]+/, {line:0, ch:0})
        if (cc.findNext())
        {
            cm.setSelection(cc.from(), cc.to())
        }
...
extraKeys = {
    ...
    "Ctrl-F":function (cm) {
        console.error("about to findPersistent")
...
console.error(" setting extrakeys to ")
console.error(extraKeys)
