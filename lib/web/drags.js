...
newOverlayWidget = function (line, ch, w, h) {
    var div = $('<div class="Field-remoteWidgetOverlay" style="width:' + w + 'px; height:' + h + 'px;"></div>')[0];

    var selection_start = cm.getCursor(true);
    var selection_end = cm.getCursor(false);

    var widget = cm.addWidget({line: line, ch: ch}, div);
    ...
    // this is surely wrong -- line and ch will have changed
    // div.serialization_1 = "newOverlayWidget("+line+","+ch+"," + w + ", " + h + ")";

    return {"div": div, "canvas": canvas, "mark": widget};
}

newWidgetCanvas = function (w, h) {
    ...
    // this is surely wrong -- selection end is not going to be correct
    div.serialization_1 = "newWidgetCanvas(" + w + ", " + h + ")";

    return canvas;
}

newMarkingCanvas = function (start, stop, w, h) {
...
newTextBox = function(start, stop, w, h, initialValue) {

    var uid = ""+generateUUID()

    var textbox = "<input type='text' name='"+uid+"' id='"+uid+"' value='"+initialValue+"'>";

    var div = $('<div class="Field-remoteTextBoxInline" style="position:relative; padding:0px; padding-left:0px; overflow:visible; margin:0px; display: inline-block; vertical-align:middle; width:' + w + 'px; height:' + h + 'px;">"+textbox+"<div class="Field-closeBoxInline">&#x2715;</div></div>')[0];
    ...
    return {"div": div, "id":uid, /*"canvas": canvas,*/ "mark": widget};
}
...
dragThang = function (into, get, setAndGet, diameter, onUp) {

    var shapes = into.circle(0, 0, diameter / 2);
    // var shapes = into.rect(0, 0, diameter,diameter);
    ...
    var move = function (dx, dy) {
        ...
        console.log("move " + ddx + " " + ddy + " " + shapes.ox + " " + dx)

        constrained = setAndGet([shapes.ox, shapes.oy], [shapes.wx, shapes.wy], [ddx, ddy]);
    ...
    var up = function () {
        ...
        if (onUp) {
            var v = onUp();
            console.log(" ran onup and got " + v)
            if (v) {
                console.log(" setting to " + v[0] + " " + v[1]);
                internalSet(shapes, v[0], v[1])
            }
        }
    };
    ...
    shapes.move = move;
    shapes.internalSet = internalSet
    shapes.mouseup(up)
...
makeSlider = function (rrRaph, w, h, get, setAndConstrain, onUp) {
    for (var q = 1; q < 10; q++)
    ...
    oo = dragThang(
        ...
        function () {
            if (onUp) {
                var u = onUp()
                console.log(" ran onUp again got " + u);
                return [insetW + u * (w - insetW * 2), h / 2]
            }
            else {
                return
            }
...
makeDifferentialSlider = function (rrRaph, w, h, get, setAndConstrain, onUp) {
    for (var q = 1; q < 10; q++)
        rrRaph.path("M" + (insetW + (q / 10.0) * (w - insetW * 2)) + "," + 1 + "L" + (insetW + (q / 10.0) * (w - insetW * 2)) + "," + (h)).attr({
            stroke: "#222",
            "stroke-width": 1
    ...
    console.log(" adding mouse down to ")
    ...
    re.mousedown(function (e) {
        ...
        console.log(" move to " + f + " " + e.offsetX)
        oo.ox = w / 2;
        oo.oy = h / 2;
        oo.wx = w / 2;
        oo.wy = h / 2;
        ...
        var ee = new MouseEvent(e.type, e)
        oo[0].dispatchEvent(ee)

        e.preventDefault()
    })

    re.mouseup(function (e) {
        oo.internalSet(oo, w / 2, h / 2)
        console.log(" up!?")
        e.preventDefault()
    })
...
var numDP = function (n) {
    if (n.indexOf(".") == -1) return -1;
    return n.length - n.indexOf(".");
}

//makeSlider(rrRaph, w, h, function(){return 0.5}, function(o,p,n){_field.log(n);return n})

insertFloatHelper = function (onWord) {

    // var f = cm.findWordAt(onWord);
    // var selection_start = f.anchor
    // var selection_end = f.head

    var h = cm.getTokenAt(onWord)
    var selection_start = {line: onWord.line, ch: h.start}
    var selection_end = {line: onWord.line, ch: h.end}

    var w = 150;
    var h = 13;

    console.log(" inserting a new marking canvas into the editor between " + stringify(selection_start));
    canvas = newOverlayWidget(selection_start.line, selection_start.ch, w, h);
    ...
    set = function (o, p, x) {
        ...
        var text = cm.getDoc().getRange(s, e)

        console.log(" text is " + text);

        var currentDP = numDP(text);

        var nextVal = "" + (Number.parseFloat(text) + 4*(Math.abs(Number.parseFloat(text)) + Math.pow(10,-currentDP)) * (x - p)).toFixed(Math.max(2, currentDP-1))

        while (numDP(nextVal) > currentDP && nextVal.charAt(nextVal.length - 1) == '0')
            nextVal = nextVal.substring(0, nextVal.length - 1)

        if (nextVal.endsWith(".") && !text.endsWith(".")) nextVal = nextVal.substring(0, nextVal.length - 1);
        ...
        if (text!=nextVal) {
            cm.getDoc().replaceRange(nextVal, s, e);

            f = findEnclosingPathForLine(onWord.line);
            if (f != null) {
    ...
    makeDifferentialSlider(canvas.canvas, w, h, get, set, up);
...
var liveTokenAt = null
var liveSliderAt = null
...
cm.on("cursorActivity", function () {
    ...
    if (!cm.getCursor()) {
        ...
        try {
            var h = cm.getTokenAt(cm.getCursor())
            var txt = cm.getDoc().getRange({line: cm.getCursor().line, ch: h.start}, {
                line: cm.getCursor().line,
                ch: h.end
            })
            var flt = Number.parseFloat(txt)
            if (!Number.isNaN(flt))
                live = true
        }
        catch (e) {
    ...
    if (live) {
        var h = cm.getTokenAt(cm.getCursor())
        if (liveTokenAt == null || (liveTokenAt.start != h.start || liveTokenAt.end != h.end)) {
            if (liveSliderAt != null)
                liveSliderAt.div.remove()

            liveSliderAt = insertFloatHelper(cm.getCursor())
            liveTokenAt = h
        }
    }
    else {
