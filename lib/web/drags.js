...
window.newOverlayWidget = function (line, ch, w, h) {
...
window.newTextBox = function (start, stop, w, h, initialValue) {
...
var dragThang = function (into, get, setAndGet, diameter, onUp) {
...
var makeSlider = function (rrRaph, w, h, get, setAndConstrain, onUp) {
    for (var q = 1; q < 10; q++)
    ...
    var p = rrRaph.path("M" + insetW + "," + (h / 2) + "L" + (w - insetW) + "," + (h / 2))
    p.attr({
        stroke: "#000",
        "stroke-width": 1
    });
...
var makeDifferentialSlider = function (rrRaph, w, h, get, setAndConstrain, onUp) {
    for (var q = 1; q < 10; q++)
        rrRaph.path("M" + (insetW + (q / 10.0) * (w - insetW * 2)) + "," + 1 + "L" + (insetW + (q / 10.0) * (w - insetW * 2)) + "," + (h)).attr({
            stroke: "#222",
            "stroke-width": 1,
...
var insertFloatHelper = function (onWord) {
    ...
    var canvas = newOverlayWidget(selection_start.line, selection_start.ch, w, h);
    ...
    var set = function (o, p, x) {
        ...
        var nextVal = "" + (parseFloat(text) + 4 * (Math.abs(parseFloat(text)) + Math.pow(10, -currentDP)) * (x - p)).toFixed(Math.max(2, currentDP - 1))
        ...
        if (text != nextVal) {
           ...
           var f = findEnclosingPathForLine(onWord.line);
            if (f != null) {
    ...
    cm.on("cursorActivity", function () {
        ...
        if (!cm.getCursor()) {
            ...
            try {
                ...
                var flt = parseFloat(txt)
                console.log("number is '" + flt + "'")
                if (!isNaN(flt))
                    live = true
            }
            catch (e) {
