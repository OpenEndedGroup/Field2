...
function Run_Selection_variant(variant) {
    ...
    _field.sendWithReturn("execution.fragment" + variant, {
        ...
        if (d.type == 'error') {
            ...
            appendRemoteOutputToLine(anchorLine, d.line + " : " + d.message, "Field-remoteOutput", "Field-remoteOutput-error", 0, d.tick)
        }
        else {

            if (d.tick)
                clearOutputsOlderThan(0, cm.lineCount(), d.tick)
            appendRemoteOutputToLine(anchorLine, d.message, "Field-remoteOutput-error", "Field-remoteOutput", 1, d.tick)
        }
    });
}
