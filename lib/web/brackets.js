...
function makePathForHandles(h1, h2, level, disabled, annotation) {
	...
	if (ps) {
		...
		path.attr({"fill-opacity": (disabled ? 0.2 : 0.9)});
...
updateAllBrackets = updateAllBrackets0; //debounce(updateAllBrackets0, 100, true);
updateAllBrackets();

var currentErrorLine = null;

removeCurrentErrorLine = function()
{
    if (currentErrorLine) {
...
globalCommands.push({
	...
	"callback": function () {
		...
		if (currentBracket != null) {
			...
			currentBracket.attr({"fill-opacity": (currentBracket.disabled ? 0.2 : 0.9)});
