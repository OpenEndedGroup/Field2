...
public abstract class CefRequest {
	...
	public enum TransitionType {
		/**
		 * Source is a link click or the JavaScript window.open function. This is also the default value for requests like sub-resource loads
		 * that are not navigations.
		 */
		TT_LINK(0),

		/**
		 * Source is some other "explicit" navigation action such as creating a new browser or using the LoadURL function. This is also the
		 * default value for navigations where the actual type is unknown.
		 */
		TT_EXPLICIT(1),

		/**
		 * Source is a subframe navigation. This is any content that is automatically loaded in a non-toplevel frame. For example, if a page
		 * consists of several frames containing ads, those ad URLs will have this transition type. The user may not even realize the content
		 * in these pages is a separate frame, so may not care about the URL.
		 */
		TT_AUTO_SUBFRAME(3),

		/**
		 * Source is a subframe navigation explicitly requested by the user that will generate new navigation entries in the back/forward
		 * list. These are probably more important than frames that were automatically loaded in the background because the user probably
		 * cares about the fact that this link was loaded.
		 */
		TT_MANUAL_SUBFRAME(4),

		/**
		 * Source is a form submission by the user. NOTE: In some situations submitting a form does not result in this transition type. This
		 * can happen if the form uses a script to submit the contents.
		 */
		TT_FORM_SUBMIT(7),

		/**
		 * Source is a "reload" of the page via the Reload function or by re-visiting the same URL. NOTE: This is distinct from the concept of
		 * whether a particular load uses "reload semantics" (i.e. bypasses cached data).
		 */
		TT_RELOAD(8);
		...
		/**
		 * Any of the core values above can be augmented by one or more qualifiers defined as TransitionFlags. These qualifiers further define
		 * the transition.
		 */
	...
	public static final class CefUrlRequestFlags {
		...
		/**
		 * If set cookies may be sent with the request and saved from the response. UR_FLAG_ALLOW_CACHED_CREDENTIALS must also be set.
		 */
		...
		/**
		 * If set 5XX redirect errors will be propagated to the observer instead of automatically re-tried. This currently only applies for
		 * requests originated in the browser process.
		 */
		public static final int UR_FLAG_NO_RETRY_ON_5XX = 1 << 7;
	}

	/**
	 * Transition qualifiers. Any of the core values above can be augmented by one or more qualifiers. These qualifiers further define the
	 * transition.
	 */
	...
	/**
	 * Get the transition type for this request. Only available in the browser process and only applies to requests that represent a main frame or
