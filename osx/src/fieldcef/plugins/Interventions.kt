...
class Interventions(val root: Box) : Box(), IO.Loaded {
    ...
    companion object {
        @JvmStatic
        fun coversChildren(trac: Box) {
            trac.properties.putToMap(Boxes.insideRunLoop, "main.__updatesize__", Supplier<Boolean> {

                // cache, no frame change

                var mi = Double.POSITIVE_INFINITY
                var mx = Double.NEGATIVE_INFINITY
                trac.both().apply(trac).forEach {
                    mi = Math.min(mi, it.properties.get(frame).x.toDouble())
                    mx = Math.max(mx, it.properties.get(frame).xw.toDouble())
                }

                val f = (trac get frame);
                if (f!!.x.toDouble() != mi || f!!.xw.toDouble() != mx) {
                    val f2 = Callbacks.frameChange(trac, Rect(mi, f.y.toDouble(), mx - mi, f.h.toDouble()))
                    trac.properties.put(frame, f2)
                }
