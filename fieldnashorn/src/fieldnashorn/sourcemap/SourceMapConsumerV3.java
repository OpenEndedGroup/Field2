...
//            lineCount = sourceMapRoot.getInt("lineCount");
            ...
            lines = new ArrayList<ArrayList<Entry>>();
    ...
    public Collection<Mapping.OriginalMapping> getReverseMapping(String originalFile, int line, int column) {
        ...
        Map<Integer, Collection<Mapping.OriginalMapping>> sourceLineToCollectionMap = reverseSourceMapping.get(originalFile);
    ...
    private Mapping.OriginalMapping getOriginalMappingForEntry(Entry entry) {
        if (entry.getSourceFileId() == UNMAPPED) {
            ...
            Mapping.OriginalMapping.Builder x = Mapping.OriginalMapping.newBuilder().setOriginalFile(sources[entry.getSourceFileId()]).setLineNumber(entry.getSourceLine() + 1).setColumnPosition(entry.getSourceColumn() + 1);
    ...
    private void createReverseMapping() {
        reverseSourceMapping = new HashMap<String, Map<Integer, Collection<Mapping.OriginalMapping>>>();
                ...
                for (Entry entry : entries) {
                    if (entry.getSourceFileId() != UNMAPPED && entry.getSourceLine() != UNMAPPED) {
                        ...
                        if (!reverseSourceMapping.containsKey(originalFile)) {
                            reverseSourceMapping.put(originalFile, new HashMap<Integer, Collection<Mapping.OriginalMapping>>());
                        }

                        Map<Integer, Collection<Mapping.OriginalMapping>> lineToCollectionMap = reverseSourceMapping.get(originalFile);

                        int sourceLine = entry.getSourceLine();

                        if (!lineToCollectionMap.containsKey(sourceLine)) {
                            lineToCollectionMap.put(sourceLine, new ArrayList<Mapping.OriginalMapping>(1));
                        }
