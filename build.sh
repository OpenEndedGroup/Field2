...
if [ ! -d kotlinc ]; then
    ...
    curl -L https://github.com/JetBrains/kotlin/releases/download/v1.2.31/kotlin-compiler-1.2.31.zip > kotlinc.zip
    echo -- decompressing --
    unzip kotlinc.zip
    echo -- complete --
fi
...
javac -source 1.9 -target 1.9 -Xlint:-deprecation -Xlint:-unchecked -classpath "../agent/lib/*"  @agent_source -d agent_classes/
cd agent_classes
...
echo -- 1/2 - compiling classes --
javac -source 1.9 -target 1.9 -Xlint:-deprecation -Xlint:-unchecked -classpath "../linker/lib/*"  @linker_source -d linker_classes/
cd linker_classes
...
javac -Xlint:-deprecation -Xlint:-unchecked -XDignore.symbol.file -source 1.9 -target 1.9 --add-opens java.desktop/com.apple.eawt=ALL-UNNAMED -classpath "field_agent.jar:field_linker.jar:../lib/jars/*:../lib/jars/orientdb/*:../osx/lib/jars/*:classes/"  @source -d classes/
